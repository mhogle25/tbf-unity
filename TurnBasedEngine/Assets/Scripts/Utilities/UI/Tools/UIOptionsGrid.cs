...
namespace BF2D.UI
{
    public class UIOptionsGrid : UIUtility
    {
        ...
        public void SetCursorAtHead()
        {
            if (this.grid is null)
            {
                Debug.LogError("[UIOptionsGrid] Tried to set the cursor at head but the grid was null");
            }
        ...
        public void InvokeEvent(InputButton inputButton)
        {
            if (this.grid is null)
            {
                Debug.LogError($"[UIOptionsGrid] Tried to invoke the {inputButton} event at position ({this.cursorPosition.x}, {this.cursorPosition.y}) but the grid was null");
                return;
            }
        ...
        private void SetCursorAtPosition(Vector2Int cursorPosition, bool value)
        {
            if (this.grid is null)
            {
                Debug.LogWarning($"[UIOptionsGrid] Tried to set the cursor to {value} at position ({cursorPosition.x}, {cursorPosition.y}) but the grid was null");
                return;
            }
